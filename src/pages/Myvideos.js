...
export function VideoDetails({videos,deleteVideo}){
    ...
    return<>
        <div id="right-side">
                    <div id='banner'>
                        <img src={videoId?`http://localhost:9000/Thumbnails/${video?.Thumbnail}`:`http://localhost:9000/Thumbnails/${videos[0]?.Thumbnail}`} alt="Thumbnail" />
                    </div>
                    <div className="details">
                        <div className="first-row">
                            ...
                            <div id="movie-details">
                                ...
                                <span id="duration">{videoId?video?.duration:videos[0]?.duration} Mins</span>
                                <span id="views">{videoId?video?.views:videos[0]?.views} Views</span>
                            </div>
                        </div>
                        ...
                        <div className="third-row">
                            ...
                            <div className="visibility">
                                <h3>Visibility</h3>
                                <select value={videoId?video?.Visibility:videos[0]?.Visibility}>
                        ...
                        <div className="fourth-row">
                            <button id='delete' onClick={()=>deleteVideo(videoId)}>Delete</button>
                            <button id='save'onClick={()=>navigate('/')}>Save</button>
                        </div>
                    </div>
                </div>
    </>
    ...
    let deleteVideo=(videoId)=>{
        ...
        .then((res)=>{
            if(res.data){
                ...
                let remainingVideos=myvideos.filter((video)=>video._id!==deletedVideo._id)
            ...
            <section>
                <div id="left-side">
                    ...
                    <div className="bottom">
                        {myvideos.map((item)=><Link to={`${item._id}`}><VideoItem key={item._id} item={item}/></Link>)}
                    </div>
                </div>
